...
public:
  ...
  const size_t len_ethernet_header = 14;

  // bool
  // getEthernetHeader(const Buffer &packet, struct ethernet_hdr &e_hdr);

  bool
  handleArpPacket(const Buffer &packet, const ethernet_hdr &e_hdr, const Interface* inInterface);

  bool
  handleIpPacket(const Buffer &packet, const ethernet_hdr &e_hdr, const Interface* inInterface);
